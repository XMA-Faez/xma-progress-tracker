...
export default function AdminDashboardContent({ clients: initialClients, pinnedClientIds, userId, teamMembers }: AdminDashboardContentProps) {
  ...
  useEffect(() => {
    localStorage.setItem('selectedTeamMembers', JSON.stringify(selectedTeamMembers))
  }, [selectedTeamMembers])
  ...
  const handleClientAssignmentChange = async (clientId: string, teamMemberId: string | null) => {
    ...
    const updatedClients = clients?.map(client => {
      if (client.id === clientId) {
        const assignedMember = teamMemberId ? teamMembers.find(m => m.id === teamMemberId) : null
        return {
          ...client,
                  ...
                  <div className="absolute top-full mt-2 right-0 w-64 bg-slate-800 rounded-lg shadow-xl border border-slate-700 z-50">
                    <div className="p-2">
                      <div className="px-3 py-2 border-b border-slate-700">
                        <h3 className="text-sm font-semibold text-white">Select Team Members</h3>
                        <p className="text-xs text-slate-400 mt-1">Choose who to show in Kanban view</p>
                      </div>
      ...
      {(!clients || clients.length === 0) ? (
        ...
        <motion.div variants={itemVariants}>
          <ClientKanbanView
            ...
            teamMembers={selectedTeamMembers.length > 0 
