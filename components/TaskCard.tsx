...
export function TaskCard({ task, onStatusUpdate, showClient = false }: TaskCardProps) {
  ...
  return (
    <div className={`bg-gray-900/50 rounded-lg border-l-4 ${getTaskTypeColor(task.type)} p-4 hover:bg-gray-900/70 transition-colors`}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1">
          <h4 className="font-medium text-white text-sm mb-1">{task.name}</h4>
          {showClient && task.clients?.name && (
            <p className="text-gray-400 text-xs mb-1">{task.clients.name}</p>
          )}
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>•</span>
            <span className="text-gray-400">{getAssignedNames()}</span>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 top-full mt-1 w-48 bg-gray-800 rounded-lg shadow-xl border border-gray-700 z-10">
              <div className="py-1">
                {statusOptions.map(option => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-700 transition-colors ${
                      task.status === option.value ? 'bg-gray-700 text-white' : 'text-gray-300'
                    }`}
                  >
                    {option.label}
                  </button>
